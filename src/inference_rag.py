...
def generate_profile(query: str, history: List[Message] = []) -> str:
    ...
    if model is None or tokenizer is None:
        ...
        q = full_text.lower()
        # NOTE: Mock mode — returns a plausible numeric Semantic ID (C1-C2-C3-ITEM_IDX format)
        # Real IDs are generated by build_semantic_ids.py (TF-IDF + SVD 128d + KMeans 16x16x16)
        # These mock values are hardcoded plausible cluster addresses, NOT real lookup results.
        # Format: C1(00-15)-C2(00-15)-C3(00-15)-ITEM(000-NNN)
        if "ital" in q:                          return "03-12-07-042"  # Italian cluster
        if "franc" in q or "french" in q:        return "01-08-03-015"  # French cluster
        if "argentin" in q:                      return "09-05-11-028"  # Argentinian cluster
        if "port" in q:                          return "07-14-02-006"  # Portuguese cluster
        if "spain" in q or "spanish" in q:       return "05-09-13-033"  # Spanish cluster
        return "02-06-10-019"  # Default: US/generic cluster mock ID
